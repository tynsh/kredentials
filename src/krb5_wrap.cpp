...
namespace krb5{ 
    ...
    const std::string principal::getData(const int i) const{
        if((i>=0)&&(i < getDataLength() )) {
            ...
            return std::string(_principal->data[i].data, _principal->data[i].length);
    ...
    std::string tixmgr::readPass(int length){
        char buf[length+1];
        int fd=dup(0);
        FILE* fp=fdopen(fd,"r");
        ...
        while (!(iter.error())){
            ...
            if(pserver) {
                ...
                if (noTix && server.getDataLength() == 2 &&
                        std::string(server.getRealm())==me.getRealm() &&
                        std::string(server.getData(0))=="krbtgt" &&
                        std::string(server.getData(1))==me.getRealm() &&
    ...
    bool tixmgr::passGetCreds(const std::string& pass){
        creds my_creds(ctx);
        std::shared_ptr<principal> osMe(NULL);
        krb5::principal* pme=cc.getPrincipal();
        ...
        kerror=krb5_get_init_creds_password(ctx(), my_creds(), me(),
                            ppass, 0, 0,
                            0,
                            0,
                            NULL);
